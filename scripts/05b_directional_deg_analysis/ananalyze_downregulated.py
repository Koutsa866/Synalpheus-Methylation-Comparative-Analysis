#!/usr/bin/env python3
"""
Directional DEG Analysis: Down-Regulated Genes
Tests if S. chacei promoter methylation predicts DOWN-REGULATION in target species.

Biological Question:
  Does promoter methylation in S. chacei predict which genes are REPRESSED 
  (down-regulated) in the social brains of S. elizabethae and S. brooksi?

Method:
  - Classification: DOWN = padj < 0.05 AND log2FoldChange < 0
  - Background: All other genes (up-regulated + non-significant)
...
Outputs:
  - Results/05b_directional_analysis/downregulated/brooksi_downregulated_contingency.csv
  - Results/05b_directional_analysis/downregulated/elizabethae_downregulated_contingency.csv
...
def run_downregulated_analysis(species, ortho_path, dea_path):
    """
    Analyze enrichment of methylated genes in DOWN-REGULATED genes.
    ...
    print(f"  Down-regulated (padj < 0.05 & log2FC < 0): {((dea_df[padj_col] < 0.05) & (dea_df[lfc_col] < 0)).sum()}")
    ...
    def classify_expression(transcript_id):
        ...
        if padj < 0.05 and lfc < 0:  # DOWN-REGULATED
            return 'DOWN'
        else:
            return 'Background'
    ...
    for row in ['Methylated', 'Unmethylated']:
        if row not in ctable.index: 
            ctable.loc[row] = [0, 0]
    for col in ['DOWN', 'Background']:
        if col not in ctable.columns: 
            ctable[col] = 0
    
    ctable = ctable.loc[['Methylated', 'Unmethylated'], ['DOWN', 'Background']]
    ...
    if (ctable < 5).any().any():
        ...
        a, b = ctable.iloc[0, 0], ctable.iloc[0, 1]  # Meth-DOWN, Meth-Background
        c, d = ctable.iloc[1, 0], ctable.iloc[1, 1]  # Unmeth-DOWN, Unmeth-Background
    ...
    if p_val < 0.05:
        ...
        print(f"  ✓ SIGNIFICANT: Methylated genes are {direction} in down-regulated genes (p < 0.05)")
    else:
        print(f"  ✗ NOT SIGNIFICANT: No association between methylation and down-regulation")
    print(f"{'='*60}\n")
...
if __name__ == "__main__":
    print("\n" + "="*60)
    print("DOWN-REGULATED GENE ANALYSIS")
    print("="*60)
    
    # Create output directory
    os.makedirs('Results/05b_directional_analysis/downregulated', exist_ok=True)
    
    results = []
    
    # Analyze S. brooksi
    results.append(run_downregulated_analysis(
        "brooksi", 
        "Results/05_ortholog_analysis/chacei_vs_brooksi.tsv", 
        "Data/transcriptomics/deseq2_results/S.brooksi_brain_DEA_wholetable.tabular"
    ))
    ...
    summary_df.to_csv('Results/05b_directional_analysis/downregulated/downregulated_summary_statistics.csv', index=False)
    print("  ✓ Results/05b_directional_analysis/downregulated/downregulated_summary_statistics.csv")
    
    # 2. Save contingency tables
    for r in results:
        ctable = r['contingency_table']
        ctable.to_csv(f"Results/05b_directional_analysis/downregulated/{r['species']}_downregulated_contingency.csv")
        print(f"  ✓ Results/05b_directional_analysis/downregulated/{r['species']}_downregulated_contingency.csv")
    
    print("\n✓ Down-regulated analysis complete!")
